...
# ABSTRACT: Base role for Giddy collection objects.
...
has 'db' => (is => 'ro', isa => 'Giddy::Database', required => 1, weak_ref => 1);
...
Giddy::Collection - Base role for Giddy collection objects.

=head1 SYNOPSIS

	# this module provides the main API for both Giddy::Collection::FileSystem
	# and Giddy::Collection::InMemory

	my $coll = $db->get_collection('articles'); # returns Giddy::Collection::FileSystem object
	my $res = $coll->find({ num => { '$gt' => 3 } }); # returns Giddy::Collection::InMemory object

=head1 DESCRIPTION

This class provides a base for Giddy collections. There are two types of
Giddy collections:

=over

=item 1. Real collections, which are directories holding documents
in the database. Collections are hierarchical in Giddy (see L<Giddy::Manual> for
more information). These are represented by L<Giddy::Collection::FileSystem>.

=item 2. In-memory collections, which are subsets of real collections
generated by running queries on other collections (either real or in-memory).
These are represented by L<Giddy::Collection::InMemory>.

=back

You will find most of your interaction with the Giddy database are performed
using the API provided by this module, which provides methods for finding documents, creating
documents, updating documents, removing documents, iterating over query results,
etc.

=head1 REQUIRES

Consuming classes must implement an attribute or method called "_documents"
that returns a L<Tie::IxHash> object of all documents in the collection,
...
the root directory of the database. Never has a starting slash. Since in-memory
collections are generated from real collections, they will always hold the
path of the real collection.

=head2 db

The L<Giddy::Database> object the collection belongs to. Always available.
...
A hash-ref containing loaded document structures. Not to be used externally.
...
consumes this class (which is a Moose role).
...
Returns a L<Giddy::Collection::FileSystem> object tied to the parent collection of the collection.
...
Returns a L<Giddy::Collection::FileSystem> object tied to a child-collection named C<$name>.
...
Increases the "_loc" attribute by one.
...
Loads the document stored (or returned) at index C<$index> of "_documents"
(which is either an attribute or a method, depending on whether the
collection is real of in-memory).
...
sub _load_document {
	...
	if (exists $self->_loaded->{$name}) {
		...
		my $doc;
		if ($t eq 'file') {
			$doc = $self->_load_document_file($name);
		} elsif ($t eq 'dir') {
			$doc = $self->_load_document_dir($name, $self->isa('Giddy::Collection::InMemory') ? $self->_opts->{skip_binary} : 0);
		}
...
Automatically calculates the value of the "_cached" attribute.
...
=head2 _load_document_file( $name )

Loads the document stored in the file C<$name>.
...
=head2 _load_document_dir( $name, [ $skip_binary ] )

Loads the document stored in the directory C<$name>.
...
sub _load_document_dir {
	...
	$doc->{_has_many} = [sort grep { $self->db->_is_collection($self->_path_to($name, $_)) } $self->db->_list_dirs($self->_path_to($name))];
...
Finds documents in the collection the match the provided name query. If
a query is not provided, all documents will match. Returns documents
found in a Tie::IxHash object.
...
sub _match_by_name {
	...
	if (!ref $name || (ref $name eq 'HASH' && scalar keys %$name == 1 && $name->{'$eq'})) {
		...
		if ($self->isa('Giddy::Collection::InMemory')) {
		...
		foreach (grep { doc_matches({ _name => $_ }, { _name => $name }) } $self->_documents->Keys) {
...
Finds documents that match the provided query object. If not provided,
all documents will match. Returns a Tie::IxHash object of all the results.
...
Stores the document C<$filename> and its attributes in the filesystem.
